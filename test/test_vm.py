...
def test_v_mfma_f32_32x32x8f16():
    mat_a_ref = np.zeros((32, 8), dtype=np.float32)
    mat_b_ref = np.zeros((8, 32), dtype=np.float32)
    ...
    c = vm.accvgpr_to_ndarray(AccVgprRange(0, 16), 32, 32, 2)
    context = GpuContext()
    context.v_mfma_f32_32x32x8f16(
        AccVgprRange(0, 16), VgprRange(0, 2), VgprRange(2, 2), AccVgprRange(0, 16)
    )
    vm.run(context)
    d = vm.accvgpr_to_ndarray(AccVgprRange(0, 16), 32, 32, 2)
