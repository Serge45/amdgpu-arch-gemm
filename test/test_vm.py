...
def test_wgm_reorganization():
    ...
    # We generate the assembly for wgm = 4
    gemm(
        context,
        "gemm",
        "gfx90a:xnack-",
    ...
    # Let's run the VM and check coordinate mapping logic
    # Grid shape: num_workgroups_x = 8, num_workgroups_y = 8
    # Input coordinates: wg_id_x = 3, wg_id_y = 2
    vm = GcnVirtualMachine(104, 256, 64)
    ...
    vm.smem.mem[60:64] = struct.pack("f", 1.0)
    vm.smem.mem[64:68] = struct.pack("f", 1.0)
    vm.smem.mem[68:72] = int.to_bytes(8, 4, "little") # numWorkgroupX = 8
    vm.smem.mem[72:76] = int.to_bytes(8, 4, "little") # numWorkgroupY = 8
    
    # Set input workgroup IDs
    vm.s[2] = 3  # wg_id_x
    vm.s[3] = 2  # wg_id_y
    
    # Set the pointer to kernel arguments (s[0:1] = 0)
    vm.s[0] = 0
    vm.s[1] = 0
    
    # We test multiple input coordinates
    for wg_y in range(8):
        for wg_x in range(8):
            # Re-initialize only the input registers and clear memory where needed
            # (or reset VM state)
            vm_test = GcnVirtualMachine(104, 256, 64)
            # Copy memory from vm to vm_test
            vm_test.smem.mem = vm.smem.mem[:]
            
            # Setup thread IDs
            for i in range(vm_test.wavefront_size):
                vm_test.v[0][i] = i
            ...
            z = wg_x + wg_y * 8
            expected_x = (z % 4) + (z // 4 // 8) * 4
            expected_y = (z // 4) % 8
            
            assert vm_test.s[2] == expected_x, f"wg_x={wg_x}, wg_y={wg_y} mapped to x={vm_test.s[2]} instead of {expected_x}"
            assert vm_test.s[3] == expected_y, f"wg_x={wg_x}, wg_y={wg_y} mapped to y={vm_test.s[3]} instead of {expected_y}"


def test_run_sgemm_16x16x4_map_k_idx():
    from generator.generator import gemm, GemmOptimizations, GemmSolutionConfig, DataType, FunctionArgument
    gemm_config = GemmSolutionConfig(
        DataType.FP32,
        DataType.FP32,
    ...
    # Set input workgroup IDs
    vm.s[2] = 3  # wg_id_x = 3. Initial mapped_k_idx = (3 % 4) * 16 = 48.
    vm.s[3] = 2  # wg_id_y = 2
...
def test_run_sgemm_16x16x4_swizzle():
    from generator.generator import gemm, GemmOptimizations, GemmSolutionConfig, DataType, FunctionArgument
    from vm.gcn_virtual_machine import GcnVirtualMachine, VectorMemory, LocalDataShare
    import struct
    gemm_config = GemmSolutionConfig(
        DataType.FP32,
        DataType.FP32,
    ...
    opt = GemmOptimizations(1, wgm=8, plr=1, gw=1, map_k_idx=0)
    context = GpuContext()
    kern_args = [
        FunctionArgument("global_buffer", "a", None, 8),
        FunctionArgument("global_buffer", "b", None, 8),
    ...
    m, n, k = 64, 64, 64
    shared_lds = LocalDataShare(65536)
    shared_vmem = VectorMemory(1024 * 1024)
    vms = [GcnVirtualMachine(104, 256, 64) for _ in range(4)]
    for w, vm in enumerate(vms):
        vm.lds = shared_lds
        vm.vmem = shared_vmem
        vm.s[2] = 0
        vm.s[3] = 0
    ...
    np.random.seed(42)
    a_val = np.random.rand(m * k).astype(np.float32)
    b_val = np.random.rand(n * k).astype(np.float32)
    c_val = np.zeros(m * n, dtype=np.float32)

    shared_vmem.mem[a_offset:a_offset+a_size] = bytearray(a_val)
    shared_vmem.mem[b_offset:b_offset+b_size] = bytearray(b_val)
    shared_vmem.mem[c_offset:c_offset+c_size] = bytearray(c_val)

    gemm(context, "generated_gemm", "gfx90a:xnack-", gemm_config, opt, kern_args)
    ...
    for vm in vms:
        ...
        vm.smem.mem[68:72] = int.to_bytes(1, 4, "little")
        vm.smem.mem[72:76] = int.to_bytes(1, 4, "little")
        vm._accumulate_labels(context)
        vm.pc = 0
        vm.pc_end = len(context.instructions)

    vms_running = [True] * 4
    while any(vms_running):
        for w in range(4):
            if not vms_running[w]:
                continue
            vm = vms[w]
            while vm.pc < vm.pc_end:
                inst = context.instructions[vm.pc]
                inst_str = inst[0]().split(" ")[0]
                args = inst[1:]
                if hasattr(vm, inst_str):
                    getattr(vm, inst_str)(*args)
                vm.pc += 1
                if inst_str == "s_barrier":
                    break
            else:
                vms_running[w] = False

    raw_d = shared_vmem.mem[d_offset:d_offset+d_size]
    d = np.frombuffer(raw_d, dtype=np.float32).reshape(n, m)
    a_mat = a_val.reshape(k, m)
    b_mat = b_val.reshape(n, k)
    ref_d = b_mat @ a_mat
    assert np.allclose(d, ref_d, atol=1e-5)
